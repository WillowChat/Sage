...
pipeline {
    ...
    stages {
        ...
        stage('Coverage') {
            environment {
                CODECOV_TOKEN = credentials('chat.willow.sage.codecov')
            }
        ...
        stage('Deploy') {
            ...
            steps {
                ...
                sh "find build/libs -name Sage\\*${env.BUILD_NUMBER}.jar | head -n 1 | xargs -I '{}' mvn install:install-file -Dfile={} -DpomFile=build/publications/mavenJava/pom-default.xml -DlocalRepositoryPath=/var/www/maven.hopper.bunnies.io"
                sh "find build/libs -name Sage\\*sources.jar | head -n 1 | xargs -I '{}' mvn install:install-file -Dfile={} -Dclassifier=sources -DpomFile=build/publications/mavenJava/pom-default.xml -DlocalRepositoryPath=/var/www/maven.hopper.bunnies.io"
            }
        }
    }
}
